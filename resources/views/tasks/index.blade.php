...
<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
            ...
            <p class="text-gray-600">Manage and assign work to departments and employees</p>
        </div>
        <a href="{{ route('tasks.create') }}" 
           class="px-4 py-2 text-white rounded-lg hover:opacity-90 transition flex items-center gap-2 w-fit"
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-6">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="flex items-end gap-2">
                ...
                <a href="{{ route('tasks.index') }}" class="px-4 py-2 border rounded-lg">Clear</a>
            </div>
        </form>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow group {{ $task->isOverdue() ? 'border-red-300 ring-1 ring-red-100' : '' }}">
                <div class="p-5 border-b border-gray-100">
                    <div class="flex items-start justify-between mb-3">
                        <span class="text-xs font-mono text-gray-500 bg-gray-100 px-2 py-1 rounded">{{ $task->task_code }}</span>
                        <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <a href="{{ route('tasks.show', $task) }}" class="text-blue-600 hover:text-blue-800 p-1">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                </svg>
                            </a>
                            <a href="{{ route('tasks.edit', $task) }}" class="text-green-600 hover:text-green-800 p-1">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                </svg>
                            </a>
                            <form action="{{ route('tasks.destroy', $task) }}" method="POST" onsubmit="return confirm('Delete?')" class="inline">
                                @csrf @method('DELETE')
                ...
                <div class="px-5 py-3 bg-gray-50 rounded-b-xl flex items-center justify-between text-sm">
                    <div class="flex items-center gap-2">
                        @if($task->assignedEmployee)
                            <div class="w-6 h-6 rounded-full bg-green-100 text-green-700 flex items-center justify-center text-xs font-semibold">
                                {{ substr($task->assignedEmployee->first_name, 0, 1) }}
                            </div>
                            <span class="text-gray-600">{{ $task->assignedEmployee->first_name }}</span>
                        @else
                    ...
                    <div class="flex items-center gap-1 text-gray-500">
                        @if($task->due_date)
                            <span class="{{ $task->isOverdue() ? 'text-red-600 font-medium' : '' }}">{{ $task->due_date->format('M d') }}</span>
                        @else
                            <span>—</span>
                        @endif
                    </div>
        ...
        @empty
            <div class="col-span-full text-center py-16">
                ...
                <a href="{{ route('tasks.create') }}" class="inline-block px-4 py-2 text-white rounded-lg" style="background-color: #0C521C;">Create Task</a>
            </div>
        @endforelse
    </div>
