                        ...
                        <div>
                            @if($todayAttendance)
                                <p class="text-gray-900 font-semibold">
                                    {{ $todayAttendance->time_in ? 'Checked In' : 'Not Checked In' }}
                                </p>
                                <p class="text-gray-500 text-sm">
                                    {{ $todayAttendance->time_in ? \Carbon\Carbon::parse($todayAttendance->time_in)->format('h:i A') : '--:--' }} 
                ...
                <div class="mt-3 sm:mt-0">
                    @if($todayAttendance && !$todayAttendance->time_out)
                        <form action="{{ route('attendance.clock-out') }}" method="POST">
                            ...
                            <button type="submit" class="px-8 py-3 text-white rounded-lg hover:opacity-90 transition font-semibold text-base" style="background-color: #0C521C;">
                                ⏰ Clock Out
                            </button>
                        </form>
...
@endif
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm text-center">
            <p class="text-3xl font-semibold text-green-600 mb-1">{{ $myAttendanceSummary['present'] ?? 0 }}</p>
            <p class="text-sm text-gray-600">Days Present</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm text-center">
            <p class="text-3xl font-semibold text-red-500 mb-1">{{ $myAttendanceSummary['absent'] ?? 0 }}</p>
            <p class="text-sm text-gray-600">Days Absent</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm text-center">
            <p class="text-3xl font-semibold text-yellow-600 mb-1">0</p>
            <p class="text-sm text-gray-600">Late Arrivals</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm text-center">
            <p class="text-3xl font-semibold text-blue-500 mb-1">{{ $myAttendanceSummary['on_leave'] ?? 0 }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('tasks.my') }}" class="text-sm text-green-600 hover:underline">View All</a>
            </div>
            @if($myTasks->count() > 0)
                <div class="space-y-3">
                    @foreach($myTasks->take(4) as $task)
                        <div class="flex items-center justify-between p-3 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                            <div>
                                ...
                                <p class="text-xs text-gray-600">{{ $task->department->name ?? '' }}</p>
                            </div>
            ...
            <div class="space-y-4">
                <div class="flex items-center justify-between p-4 bg-green-50 rounded-lg">
                    <div>
                        ...
                        <p class="text-2xl font-semibold text-green-900">{{ $myAttendanceSummary['present'] ?? 0 }}</p>
                    </div>
                    <div class="w-12 h-12 rounded-full bg-green-200 flex items-center justify-center">
                        <span class="text-lg font-semibold text-green-700">{{ $myAttendanceSummary['present'] ?? 0 }}</span>
                    </div>
                </div>
                <div class="flex items-center justify-between p-4 bg-yellow-50 rounded-lg">
                    <div>
                        <p class="text-sm text-yellow-700 font-medium">On Leave</p>
                        <p class="text-2xl font-semibold text-yellow-900">{{ $myAttendanceSummary['on_leave'] ?? 0 }}</p>
                    </div>
                    <div class="w-12 h-12 rounded-full bg-yellow-200 flex items-center justify-center">
                        <span class="text-lg font-semibold text-yellow-700">{{ $myAttendanceSummary['on_leave'] ?? 0 }}</span>
                    </div>
                </div>
                <div class="flex items-center justify-between p-4 bg-red-50 rounded-lg">
                    <div>
                        <p class="text-sm text-red-700 font-medium">Absent</p>
                        <p class="text-2xl font-semibold text-red-900">{{ $myAttendanceSummary['absent'] ?? 0 }}</p>
                    </div>
                    <div class="w-12 h-12 rounded-full bg-red-200 flex items-center justify-center">
                        <span class="text-lg font-semibold text-red-700">{{ $myAttendanceSummary['absent'] ?? 0 }}</span>
                    </div>
                </div>
            </div>
            @php
                $totalDays = ($myAttendanceSummary['present'] ?? 0) + ($myAttendanceSummary['absent'] ?? 0) + ($myAttendanceSummary['on_leave'] ?? 0);
                $attendanceRate = $totalDays > 0 ? round(($myAttendanceSummary['present'] ?? 0) / $totalDays * 100) : 0;
            @endphp
            <div class="mt-4 pt-4 border-t border-gray-200">
                <p class="text-sm text-gray-600">Attendance Rate: <span class="font-semibold text-green-600">{{ $attendanceRate }}%</span></p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-lg font-semibold text-gray-900">Leave Balance</h2>
                <a href="{{ route('leaves.my') }}" class="text-sm text-green-600 hover:underline">Apply</a>
            </div>
            <div class="space-y-4">
                @foreach([
                    ['type' => 'Annual Leave', 'remaining' => 20 - ($leaveRequests->where('type', 'annual')->where('status', 'approved')->sum('duration_days') ?? 0), 'total' => 20, 'color' => '#eab308'],
                    ['type' => 'Sick Leave', 'remaining' => 10 - ($leaveRequests->where('type', 'sick')->where('status', 'approved')->sum('duration_days') ?? 0), 'total' => 10, 'color' => '#ef4444'],
                    ['type' => 'Personal Leave', 'remaining' => 5 - ($leaveRequests->where('type', 'personal')->where('status', 'approved')->sum('duration_days') ?? 0), 'total' => 5, 'color' => '#3b82f6'],
                ] as $leave)
                    @php $pct = $leave['total'] > 0 ? round((max(0, $leave['remaining']) / $leave['total']) * 100) : 0; @endphp
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-sm text-gray-900">{{ $leave['type'] }}</span>
                            <span class="text-sm font-semibold">{{ max(0, $leave['remaining']) }}/{{ $leave['total'] }} days</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="h-2 rounded-full" style="width: {{ $pct }}%; background-color: {{ $leave['color'] }}"></div>
                        </div>
                    </div>
                @endforeach
            </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('payslips.my') }}" class="text-sm text-green-600 hover:underline">View All</a>
            </div>
            <div class="space-y-3">
                @forelse($recentPayslips ?? [] as $slip)
                    <div class="flex items-center justify-between p-3 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-sm font-medium text-gray-900">{{ $slip->period_start->format('F Y') }}</p>
                                <p class="text-xs text-gray-600">{{ $slip->period_start->format('M d, Y') }}</p>
                            </div>
                        </div>
                        <div class="text-right">
                            <p class="text-sm font-semibold text-green-600">${{ number_format($slip->net_salary) }}</p>
                            <span class="text-xs text-green-600 bg-green-100 px-2 py-0.5 rounded-full">Paid</span>
                        </div>
                    </div>
                @empty
                    <p class="text-gray-500 text-center py-8">No payslips yet</p>
                @endforelse
            </div>
        </div>
    </div>
