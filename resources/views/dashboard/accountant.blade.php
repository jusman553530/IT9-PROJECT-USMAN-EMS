...
<div>
    ...
    <div class="mb-8">
        ...
        <p class="text-gray-600">
            {{ $employee->position->title ?? 'Accountant' }} · {{ $employee->department->name ?? 'Finance' }} · {{ $employee->employee_id ?? 'N/A' }} · {{ \Carbon\Carbon::today()->format('l, F j, Y') }}
        </p>
    </div>
            ...
            <div class="flex items-center gap-6">
                ...
                @if($todayAttendance && !$todayAttendance->time_out)
                    <form action="{{ route('attendance.clock-out') }}" method="POST">
                        ...
                        <button type="submit" class="px-4 py-2 text-white rounded-lg hover:opacity-90 transition font-medium" style="background-color: #0C521C;">
                            Clock Out
                        </button>
                    </form>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('payroll.index') }}" class="text-sm text-green-600 hover:underline">View All</a>
            </div>
            <div class="space-y-3">
                @forelse($recentPayrolls as $payroll)
                    <div class="flex items-center justify-between p-3 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                        <div>
                            <p class="text-sm font-medium text-gray-900">{{ $payroll->employee->full_name ?? 'N/A' }}</p>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
            ...
            <div class="mt-4 pt-4 border-t border-gray-200">
                <p class="text-sm text-gray-600">Attendance Rate: <span class="font-semibold text-green-600">{{ $rate }}%</span></p>
            </div>
        </div>
    </div>
</div>
