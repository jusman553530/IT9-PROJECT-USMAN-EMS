...
<div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
            ...
            <p class="text-3xl font-semibold text-gray-900 text-right">${{ number_format($stats['total_payroll'], 2) }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
            <p class="text-sm text-gray-600 mb-1">This Month</p>
            <p class="text-3xl font-semibold text-gray-900 text-right">${{ number_format($stats['this_month'], 2) }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
            <p class="text-sm text-gray-600 mb-1">Pending</p>
            <p class="text-3xl font-semibold text-yellow-600 text-right">{{ $stats['pending'] }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
            <p class="text-sm text-gray-600 mb-1">Paid</p>
            <p class="text-3xl font-semibold text-green-600 text-right">{{ $stats['paid'] }}</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-6">
        <form method="GET" action="{{ route('payroll.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="flex items-end gap-2">
                ...
                <a href="{{ route('payroll.index') }}" class="px-4 py-2 border rounded-lg">Clear</a>
            </div>
        </form>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($payrolls as $payroll)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $payroll->period_start->format('M d') }} - {{ $payroll->period_end->format('M d, Y') }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                                <svg class="w-16 h-16 mx-auto mb-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2z"></path></svg>
        ...
        @if($payrolls->hasPages())
            <div class="px-6 py-4 border-t">{{ $payrolls->links() }}</div>
        @endif
    </div>
</div>
@endsection