...
<div id="sidebar" class="fixed lg:static inset-y-0 left-0 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out z-40 w-64 bg-white shadow-lg min-h-screen overflow-hidden">
    ...
    <nav class="mt-6 px-3">
        ...
        <div class="mt-6">
            <p class="px-4 text-xs font-semibold text-gray-400 uppercase tracking-wider">Important</p>
        </div>

        <a href="{{ route('payroll.index') }}" 
           class="mt-2 flex items-center px-4 py-3 text-gray-700 rounded-lg hover:bg-gray-100 {{ request()->routeIs('payroll.*') ? 'bg-green-50 text-green-700' : '' }}">
            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
   ...
   class="mt-1 flex items-center px-4 py-3 text-gray-700 rounded-lg hover:bg-gray-100 {{ request()->routeIs('expenses.*') ? 'bg-green-50 text-green-700' : '' }}">
    ...
    <span>Expenses</span>
        ...
        <a href="{{ route('attendance.my') }}" 
           class="mt-2 flex items-center px-4 py-3 text-gray-700 rounded-lg hover:bg-gray-100 {{ request()->routeIs('attendance.my') ? 'bg-green-50 text-green-700' : '' }}">
            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <a href="{{ route('leaves.my') }}" 
           class="mt-1 flex items-center px-4 py-3 text-gray-700 rounded-lg hover:bg-gray-100 {{ request()->routeIs('leaves.my') ? 'bg-green-50 text-green-700' : '' }}">
            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
